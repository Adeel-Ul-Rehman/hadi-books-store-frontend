...
const ShopContextProvider = ({ children }) => {
  ...
  const addToCart = async (productId, format = null, quantity = 1) => {
    ...
    try {
      ...
      const data = await apiRequest('post', '/api/cart/add', { productId, format, quantity });
      if (data.success) {
  ...
  const removeFromCart = async (productId, format = null) => {
    ...
    try {
      ...
      const data = await apiRequest('delete', '/api/cart/remove', { productId, format });
      if (data.success) {
        await fetchCart();
        return true;
      }
  ...
  const updateCart = async (productId, format = null, quantity) => {
    ...
    try {
      ...
      const data = await apiRequest('put', '/api/cart/update', { productId, format, quantity });
      if (data.success) {
        await fetchCart();
        return true;
      }
